...
// QA-41 — Client Component hijo del Server Component categories/page.tsx.
// Recibe las categorías ya cargadas en el servidor (initialCategories) para que
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          ...
          {categories.map((category) => (
            <div key={category.id} className="flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <div>
                <p className="font-semibold text-slate-900">{category.name}</p>
                <p className="text-sm text-slate-500">{category.type} {category.parent_id ? `• padre ${category.parent_id}` : "• raíz"}</p>
              </div>
              <div className="flex w-full justify-center gap-2">
                <Button type="button" variant="default" size="sm" onClick={() => handleEdit(category)}>
                  <Pencil className="h-4 w-4" />
                  Editar
                </Button>
                <Button type="button" variant="default" size="sm" onClick={() => void handleDelete(category.id)}>
