...
⚠️ **Pendiente de decidir, encontrado al releer el doc**: la sección 6 ("Corrección de diseño: tarjetas de crédito") y las secciones 3.1/3.4/3.4.1 actualizadas describen un rediseño más profundo de deudas de tarjeta que el que se implementó como pulido rápido arriba — mover `interest_rate`/`minimum_payment`/`cutoff_day`/`payment_due_day` a `accounts`, dejar `debts` solo para préstamos/personales (con `archived_at` para no borrar historial), fusionar ambas fuentes en una sola lista en Deudas (no dos secciones separadas), y una zona de alertas en Dashboard que también incluya transacciones recurrentes próximas. No se ha implementado todavía — ver conversación para decidir si se aborda.
